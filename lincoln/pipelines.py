...
class LincolnPipeline:

    # Database setup
    conn = sqlite3.connect('lincoln.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `lincoln`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM lincoln WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO lincoln (date, title, link, content)"
